...
class ModelRouter:
    ...
    def get_llm(self, purpose: str, model_preference: str | None = None, **kwargs: Any) -> ChatOpenAI:
        """返回可直接给 LangChain/LangGraph 使用的 ChatModel。"""
        _ = purpose
        candidates = self._select_candidates(model_preference)
        if not candidates:
            raise RuntimeError("没有可用模型配置")
        cfg = candidates[0]
        model_kwargs = dict(cfg.extra)
        model_kwargs.update(kwargs)
        extra_body = dict(model_kwargs.pop("extra_body", {}) or {})
        model_name_lower = cfg.model_id.lower()
        # Qwen3 兼容接口在非流式调用时通常要求显式关闭 thinking。
        if "qwen3" in model_name_lower and "enable_thinking" not in extra_body:
