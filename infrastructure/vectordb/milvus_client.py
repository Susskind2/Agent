...
class MilvusManager:
    ...
    async def insert(
        ...
        def _insert() -> None:
            ...
            texts = [str(m.get("text", ""))[:65535] for m in metadata]
            document_ids = [str(m.get("document_id", ""))[:64] for m in metadata]
            chunk_indices = [int(m.get("chunk_index", 0)) for m in metadata]
            col.insert([ids, vectors, texts, document_ids, chunk_indices])
            col.flush()
